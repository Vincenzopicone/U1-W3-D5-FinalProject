...
let me = { name: "Vincenzo", surname: "Picone", age: 34 };
...
const rollTheDices = (n) => {
  ...
  sum = values.reduce((a, num) => a + num);
  return { sum, values };
};

console.log("Esercizio 8:", rollTheDices(9));
...
const howManyDays = (data) => {
  ...
  let differenzaDate = oggi.getTime() - dataInserita.getTime();
  return Math.floor(differenzaDate / (10006060 * 24));
};

console.log("Esercizio 9: La differenza è:", howManyDays(1988 - 10 - 19));

/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il tuo compleanno, falso negli altri casi.
*/

const isTodayMyBirthday = (dataCompleanno) => {
  let compleanno = new Date(dataCompleanno);
  const mioCompleanno = {
    anno: compleanno.getFullYear(),
    mese: compleanno.getMonth(),
    giorno: compleanno.getDay(),
  };
  let data = new Date();
  const dataScelta = {
  ...
  if (data.mese === mioCompleanno.mese && data.giorno === data.mioCompleanno) {
    return true;
  } else {
    return false;
  }
...
const newestMovie = (array) => {
  let filmPiuRecente = array[0];

  array.forEach((element) => {
    if (parseInt(filmPiuRecente.Year) < parseInt(element.Year)) {
